...
const JobApplication = () => {
  ...
  const [isSubmitting, setIsubbmitting ] = useState(false)
  ...
  const handleJobSubmit = async (data) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://job-me-server.onrender.com/api/v1/jobs/apply/${jobId}`,
        formData,
        {
          headers: {
      ...
      if (response.data.success) {
        setShowModal(true);
        setIsubbmitting(flase);
      }
    } catch (error) {
      console.log(error);
      toast.error(error?.response?.data);
      setIsubbmitting(false);
    }
  };
